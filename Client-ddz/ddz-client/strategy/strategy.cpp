...
Cards Strategy::firstPlay()
{
    ...
    if(hand.type() != PlayHand::Hand_Unknown)
    {
        return m_cards;
    }
    // 不是单一牌型
    ...
    // 有没有炸弹
    QVector<Cards> bombArray = findCardType(PlayHand(PlayHand::Hand_Bomb, Card::Point_begin, 0), false);
    backup.remove(bombArray);

    // 有没有飞机
    QVector<Cards> planeArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Plane, Card::Point_begin, 0), false);
    ...
    // 有没有三张点数相同的牌
    QVector<Cards> seqTripleArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Triple, Card::Point_begin, 0), false);
    ...
    // 有没有连对
    QVector<Cards> seqPairArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Seq_Pair, Card::Point_begin, 0), false);
    ...
    if(hasTriple)
    {
        if(PlayHand(seqTripleArray[0]).pt() < Card::Card_A)
        {
    ...
    {
        for(Card::CardPoint point = Card::CardPoint(Card::Point_end-1);
    ...
    else
    {
        for(Card::CardPoint point =  Card::Card_3;
             point < Card::Point_end; point = Card::CardPoint(point+1))
        {
            int pointCount = backup.pointCount(point);
            if(pointCount == 1)
            {
...
bool Strategy::whetherToBeat(Cards &cs)
{
    ...
    if(m_player->role() == pendPlayer->role())
    {
        ...
        Cards left = m_cards;
        left.remove(cs);
        if(PlayHand(left).type() != PlayHand::Hand_Unknown)
        {
            return true;
        }
        // 如果cs对象中的牌的最小点数是2,大小王 --> 不出牌
        Card::CardPoint basePoint = PlayHand(cs).pt();
    ...
    else
    {
        ...
        if((myHand.type() == PlayHand::Hand_Triple_Single || myHand.type() == PlayHand::Hand_Triple_Pair)
                && myHand.pt() == Card::Card_2)
        {
            return false;
        }

        // 如果cs是对2, 并且出牌玩家手中的牌数量大于等于10 && 自己的牌的数量大于等于5, 暂时放弃出牌
        if(myHand.type() == PlayHand::Hand_Pair && myHand.pt() == Card::Card_2
...
QVector<Cards> Strategy::findCardsByCount(int count)
{
    ...
    for(Card::CardPoint point = Card::Card_3; point < Card::Point_end; point = (Card::CardPoint)(point+1))
    {
...
{
    // 1. 得到所有顺子的组合
    QVector<Cards> allSeq = Strategy(m_player, cards).findCardType(PlayHand(PlayHand::Hand_Seq_Single, Card::Point_begin, 0), false);
    if(allSeq.isEmpty())
    {
        // 结束递归,将满足条件的顺子传递给调用者
...
QVector<Cards> Strategy::getCards(Card::CardPoint point, int number)
{
    ...
    for(Card::CardPoint pt=point; pt < Card::Point_end; pt = (Card::CardPoint)(pt + 1))
    {
        // 目的是尽量不拆分别的牌型
...
QVector<Cards> Strategy::getTripleSingleOrPair(Card::CardPoint begin, PlayHand::HandType type)
{
    ...
    if(!findCardArray.isEmpty())
    {
        ...
        QVector<Cards> cardsArray = st.findCardType(PlayHand(type, Card::Point_begin, 0), false);
...
{
    QVector<Cards> findcardsArray;
    for(Card::CardPoint point = begin; point < Card::Point_end; point = (Card::CardPoint)(point + 1))
    {
